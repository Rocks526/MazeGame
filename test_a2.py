...
class TestGameLogic(TestFunctionality):
    ...
    @skipIfFailed(TestPlayer, TestPlayer.test_set_position)
    @skipIfFailed(test_name=test_get_player_1)
    def test_new_position_w_2(self):
        """ test GameLogic.new_position changed position W """
        game = self.a2.GameLogic('game1.txt')
        game.get_player().set_position((2, 3))
        result = game.new_position('W')
        self.assertEqual(result, (1, 3))

    @skipIfFailed(TestPlayer, TestPlayer.test_set_position)
    @skipIfFailed(test_name=test_get_player_1)
    def test_new_position_s_2(self):
        """ test GameLogic.new_position changed position S"""
        game = self.a2.GameLogic('game1.txt')
        game.get_player().set_position((2, 3))
        result = game.new_position('S')
    ...
    @skipIfFailed(TestPlayer, TestPlayer.test_set_position)
    @skipIfFailed(test_name=test_get_player_1)
    def test_new_position_a_2(self):
        """ test GameLogic.new_position changed position A """
        game = self.a2.GameLogic('game1.txt')
        game.get_player().set_position((2, 3))
        result = game.new_position('A')
        self.assertEqual(result, (2, 2))
    ...
    @skipIfFailed(TestPlayer, TestPlayer.test_set_position)
    @skipIfFailed(test_name=test_get_player_1)
    def test_new_position_d_2(self):
        """ test GameLogic.new_position changed position D """
        game = self.a2.GameLogic('game1.txt')
        game.get_player().set_position((2, 3))
        result = game.new_position('D')
    ...
    def test_move_player_multiple(self):
        """ test GameLogic.move_player multiple """
        game = self.a2.GameLogic('game1.txt')
        player = game.get_player()
        game.move_player('D')
        game.move_player('D')
    ...
    @skipIfFailed(test_name=test_move_increase_on_hit)
    def test_move_increase_on_hit_2(self):
        """ test GameLogic with MoveIncrease.on_hit with moves=10 """
        _init = self.a2.MoveIncrease.__init__

        @functools.wraps(_init)
        def move_inc_init(self, moves=5):
            _init(self, 10)

        self.a2.MoveIncrease.__init__ = move_inc_init
        ...
        try:
            ...
            ret = move_increase.on_hit(game)
            self.assertIsNone(game.get_entity((6, 6)))
            self.assertEqual(player.moves_remaining(), 22)
            self.assertIsNone(ret, msg="This function should not return a value")
        finally:
            self.a2.MoveIncrease.__init__ = _init
...
class TestGameApp(TestFunctionality):
    ...
    def test_move_increase(self):
        ...
        _init = self.a2.GameLogic.__init__

        @functools.wraps(_init)
        def game_logic_init(_self, dungeon_name='game1.txt'):
            _init(_self, 'game2.txt')

        # have to patch the defaults since GameApp.__init__ doesn't take the filename
        self.a2.GameLogic.__init__ = game_logic_init

        try:
            self.assertPlay('game_move_inc.in', 'game_move_inc.out')
        finally:
            # make sure to restore __init__ even on test fail
            self.a2.GameLogic.__init__ = _init
